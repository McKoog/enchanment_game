...
class LeatherSetEffect extends ArmorSetEffect {
  ...
  double getAttackSpeedMultiplier(int enchantLevel, WeaponType weaponType) {
    if (weaponType == WeaponType.dagger) {
      return 0.90; // +10% attack speed (lower interval)
    }
    return 1.0;
  }
  ...
  List<Widget> buildSetEffectsDescription() {
    return [
      ...
      const Text(
          '+ 1 hp regen (each 5 enchantments levels will grant additional +1 hp regen)',
          style: TextStyle(
              color: AppColors.accentYellow,
              fontFamily: 'PT Sans',
              fontSize: 14)),
      const SizedBox(height: 12),
      const Text('Weapon effect:',
          style: TextStyle(
              color: AppColors.accentYellow,
              fontFamily: 'PT Sans',
              fontSize: 14,
      ...
      const SizedBox(height: 4),
      const Text('Fists: + 1 min/max damage, +1 defence',
          style: TextStyle(
              color: AppColors.error, fontFamily: 'PT Sans', fontSize: 14)),
      const SizedBox(height: 4),
      const Text('Sword: +1 defence',
          style: TextStyle(
              color: AppColors.error, fontFamily: 'PT Sans', fontSize: 14)),
      const SizedBox(height: 4),
      const Text('Dagger: + 10% attack speed, + 3% crit chance',
          style: TextStyle(
              color: AppColors.error, fontFamily: 'PT Sans', fontSize: 14)),
      const SizedBox(height: 4),
      const Text('Bow: 10% chance to trigger attack 2 time, -1 min/max damage',
          style: TextStyle(
              color: AppColors.error, fontFamily: 'PT Sans', fontSize: 14)),
    ];
  }
  ...
  List<Widget> buildActiveEffectsList(int enchantLevel, Weapon weapon) {
    ...
    effects.add(const Text('+ 10 hp',
        style: TextStyle(color: Colors.green, fontFamily: 'PT Sans')));
    effects.add(const SizedBox(height: 8));

    final regenBonus = getBonusHpRegen(enchantLevel);
    effects.add(Text('+ $regenBonus hp regen',
        style: const TextStyle(color: Colors.green, fontFamily: 'PT Sans')));
    effects.add(const SizedBox(height: 8));

    effects.add(Text('${weapon.name} Effect:',
        style: const TextStyle(color: Colors.yellow, fontFamily: 'PT Sans')));
    effects.add(const SizedBox(height: 8));

    if (weapon.weaponType == WeaponType.fist) {
      effects.add(const Text('+ 1 min/max damage',
          style: TextStyle(color: Colors.green, fontFamily: 'PT Sans')));
      effects.add(const SizedBox(height: 8));
      effects.add(const Text('+ 1 defence',
          style: TextStyle(color: Colors.green, fontFamily: 'PT Sans')));
      effects.add(const SizedBox(height: 8));
      ...
      effects.add(const Text('+ 10% attack speed',
          style: TextStyle(color: Colors.green, fontFamily: 'PT Sans')));
      effects.add(const SizedBox(height: 8));
      effects.add(const Text('+ 3% crit chance',
          style: TextStyle(color: Colors.green, fontFamily: 'PT Sans')));
      effects.add(const SizedBox(height: 8));
    } else if (weapon.weaponType == WeaponType.bow) {
      effects.add(const Text('10% chance to trigger attack 2 time',
          style: TextStyle(color: Colors.green, fontFamily: 'PT Sans')));
      effects.add(const SizedBox(height: 8));
      effects.add(const Text('- 1 min/max damage',
          style: TextStyle(color: Colors.green, fontFamily: 'PT Sans')));
