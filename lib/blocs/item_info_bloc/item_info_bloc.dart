...
class ItemInfoBloc extends Bloc<ItemInfoEvent, ItemInfoState>{
  ...
  void _showInfo(ItemInfoEvent$ShowInfo event, Emitter<ItemInfoState> emitter){
    if(state is ItemInfoState$Showed && (state as ItemInfoState$Showed).item.id == event.item.id){
  ...
  void _closeInfo(ItemInfoEvent$CloseInfo event, Emitter<ItemInfoState> emitter){
    emitter(ItemInfoState$Idle());
  }
}