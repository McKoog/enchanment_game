...
class HuntingFieldsBloc extends Bloc<HuntingFieldEvent, HuntingFieldState> {
  HuntingFieldsBloc({required final Weapon initialWeapon, required final Monster initialMonster}) : super(HuntingFieldState$PickPhase(selectedMonster: initialMonster,selectedWeapon: initialWeapon)) {
  ...
  void _selectMonster(HuntingFieldEvent$SelectMonster event,
    ...
    emitter(HuntingFieldState$PickPhase(selectedWeapon: state.selectedWeapon, selectedMonster: event.monster));
  }

  void _selectWeapon(HuntingFieldEvent$SelectWeapon event,
      Emitter<HuntingFieldState> emitter) {
    emitter(HuntingFieldState$PickPhase(selectedWeapon: event.weapon, selectedMonster: state.selectedMonster));
  }

  void _startHunting(HuntingFieldEvent$StartHunting event,
      Emitter<HuntingFieldState> emitter) {
    emitter(HuntingFieldState$HuntingStarted(selectedWeapon: state.selectedWeapon, selectedMonster: state.selectedMonster));
  }

  void _stopHunting(HuntingFieldEvent$StopHunting event,
      Emitter<HuntingFieldState> emitter) {
    emitter(HuntingFieldState$PickPhase(selectedWeapon: state.selectedWeapon, selectedMonster: state.selectedMonster));
  }
}
