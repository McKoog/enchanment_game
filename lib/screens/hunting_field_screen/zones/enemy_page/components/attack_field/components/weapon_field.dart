...
class WeaponField extends StatelessWidget {
  const WeaponField({super.key, required this.weapon, this.showBackground = true});
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final double availableSize = constraints.maxHeight < constraints.maxWidth ? constraints.maxHeight : constraints.maxWidth;
        final double margin = showBackground ? (availableSize * 0.1).clamp(8, 50) : 0;
        final double padding = showBackground ? (availableSize * 0.06).clamp(4, 27) : 0;
                ...
                ? (weapon.enchantLevel < 1
                    ...
                    : BoxDecoration(
                        ...
                        boxShadow: weapon.enchantLevel == 0
                            ? null
                            : weapon.enchantLevel > 20
                                ? [BoxShadow(color: enchantedWeaponsGlowColors[21], blurRadius: 30, spreadRadius: 15)]
                                : [BoxShadow(color: enchantedWeaponsGlowColors[weapon.enchantLevel], blurRadius: 30, spreadRadius: 15)]))
                : null,
            child: Image.asset(
              weapon.image,
